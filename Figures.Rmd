...
PopoutsfolderEXC = paste0(Popoutsfolder, "/ExcludingParalogs") #excluding
...
Plot the distribution of RAD-loci with at least one SNP at P=0.5 among populations and species for Figure 2.
...
plt<- plt + scale_fill_discrete(name="", labels=c("a) Total by population", "b) Shared with ingroup", "c) Unique per population")) #nicer legend
...
plt + xlab("Population") + ylab(expression(paste("Number of RAD-loci with at least one SNP-locus at ", italic(p), "=0.5")))
```


Plot the number of RAD-loci with at least one SNP at P=0.5 present in at least two pops/species by species
Figure X.
```{rfig.width=8, fig.height=10 }
# Get functions
library(ggplot2)
...
Plot the Allele frequency spectrum (AFS) distribution for loci in each population that are SNPs in Berberis by ploting the frequency of the major allele (P) for Figure 3. 
...
For the dataset EXCLUDING paralogs plot the PCoA of all samples and excluding El Zamorano, for Figure 4.
...
plinkfile=paste0(WD,PopoutsfolderEXC,"/AllLoci/BerAll/out.noreplicates/plink.raw")
