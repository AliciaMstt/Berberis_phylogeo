...
Plot the Site frequency spectrum (SFS) distribution for loci in each population that are SNPs in Berberis by ploting the frequency of the major allele (P) for Figure 3. 
...
plt <- plt + geom_histogram(aes(y =..density..), colour="black", fill="white", binwidth = 0.08)  +
  ...
  ylim(0,12)
plt <- plt + facet_wrap(~Pop.Name) 
plt2 <- plt + ggtitle(expression(paste("b) Excluding loci where ", italic("p"), "=0.5 in \u2265 2 pops. or spp."))) + theme(title=element_text(size=12, vjust=0.9), axis.title=element_text(size=12, vjust=0.4))
...
plt<- plt + theme_bw() 
plt<- plt + scale_x_discrete(limits=c("Aj","Iz","Ma","Pe","Tl","To","An", "Za")) #pops in desired order and excluding Out
...
# ger potential private paralogs
P0.5only1<-getP0.5(popsumstats=popsumstats, minp=1, maxp=1) 
P0.5only1<-P0.5only1[P0.5only1$P==0.5, ] #subset dataframe to loci P=0.5
...
# Plot
plt <- ggplot(data = df, aes(x = meanFst, y = priv.plg))  +      geom_point() + theme_bw() + xlab(expression(paste("mean ", italic("F") [ST], " (ortologous loci)"))) + ylab("Private potential paralogs") +
...
DRAFT For the dataset EXCLUDING all potential paralogs plot the Fst matrix as a NJ dendogram, for Figure X. 
```{r fig.width=8, fig.height=11}
#### Build nj tree based on Fst matrix
## read Fst Matrix estimated only with ortologous loci for all spp
popNames=c("Aj","An","Iz","Ma","Pe","Tl","To","Za","Out")
Fst<-read.fst_summary(file=paste0(WD,PopoutsfolderINC,"/AllLoci/BerAll/out.noreplicates/batch_1.fst_summary.tsv"), popNames=popNames)
...
require(ape)
ddg<-nj(Fst)
ddg<-root(ddg, outgroup="Out")
plot(ddg,
    type="phylogram",
    show.tip=TRUE,
    cex=0.8, underscore=TRUE,
    edge.width=2, label.offset=0.005)

#### Add number of private paralogs per branch
# load Stacks output file with populations summary statistics estimated with orthologous loci for all spp.
popsumstats <-read.sumstats(file=paste0(WD,PopoutsfolderINC,"/AllLoci/BerEQsz/out.noreplicates/batch_1.sumstats.tsv"),
  npop=9, popNames=c("Aj","An","Iz","Ma","Pe","Tl","To","Za","Out"))  
## P=0.5 PRIVATE to one population
...
#### add number of private paralogs to tip labels
# do it for each Pop to be able to adjust position
tiplabels(text=priv.ct[1,2], tip=1, bg="white", adj=-2, cex=0.7, frame="none" )
tiplabels(text=priv.ct[2,2], tip=2, bg="white", adj=-1.5, cex=0.7, frame="none" )
tiplabels(text=priv.ct[3,2], tip=3, bg="white", adj=-2, cex=0.7, frame="none" )
tiplabels(text=priv.ct[4,2], tip=4, bg="white", adj=-1.5, cex=0.7, frame="none" )
tiplabels(text=priv.ct[5,2], tip=5, bg="white", adj=-1.5, cex=0.7, frame="none" )
tiplabels(text=priv.ct[6,2], tip=6, bg="white", adj=-1, cex=0.7, frame="none" )
tiplabels(text=priv.ct[7,2], tip=7, bg="white", adj=-1.5, cex=0.7, frame="none" )
tiplabels(text=priv.ct[8,2], tip=8, bg="white", adj=-1.5, cex=0.7, frame="none" )

meanFst<-apply(Fst, 2, mean)
x<-cbind(priv.ct,meanFst)
x<-x[1:8,] #remove outgroup
plot(x$meanFst,x$freq)
qplot(x$meanFst,x$freq)
...
##### Generate datasets
### P=0.5 in at least ONE pops or spp.
P0.5atleast1<-getP0.5(popsumstats=popsumstats, minp=1, maxp=9) 
P0.5atleast1<-P0.5atleast1[P0.5atleast1$P==0.5, ] #subset dataframe to loci P=0.5
P0.5atleast1<-P0.5atleast1[!duplicated(P0.5atleast1[, c("Pop.Name", "Locus.ID")]), ]# there could be >1 SNPs at P=.5 per RAD-locus, keep only the first


### See distribution among spp and pops
require(reshape2)

# Keep only pop and Loci data
df<-P0.5atleast1[,c(1,3)]
...
PCoA_popt(dist_mt, cols.pop.key=cols.pop.key, vect.grps=labels(dist_mt), pile = TRUE, title="a) All Samples") #labels = pops
